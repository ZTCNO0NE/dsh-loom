...
export function apply(ctx, config) {
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args) {
            ...
            if (!action)
                return { accepted: false, error: 'action must be pause, cancel, or resume' };
            const jobId = typeof args.jobId === 'string' ? args.jobId : undefined;
            ...
            try {
                if (action === 'pause' || action === 'cancel') {
                    ...
                    return cleanToolResult({ accepted: true, action, ...(target.jobId ? { jobId: target.jobId } : {}), ...controlled, note: action === 'pause' ? '已暂停；不会再进入下一模型回合。resume 会创建新 attempt。' : '已取消；该 run 不可再次提交。' });
                }
                const priorJob = target.jobId ? readJson(jobPathFor(target.jobId)) : null;
                const holder = loopRunHolders.get(target.runId);
                if (!holder) {
                    return { accepted: false, runId: target.runId, error: 'resume requires an in-process loop runner; after host reload re-delegate with meta_auto(resumeRunId=...)' };
                }
                const started = loopCandidateGateway.resumeExploration(target.runId);
                if (!started.accepted)
                    return { accepted: false, exploration: started };
                if (holder.holder)
                    holder.holder.runId = started.runId;
                ...
                return cleanToolResult({ accepted: true, action, jobId: nextJobId, runId: started.runId, builderSessionId: nextJobId, resumedFromRunId: target.runId, state: 'scheduled', note: '已创建新的 immutable Builder attempt；旧 workspace/journal/artifacts 仅按 hash 只读继承。' });
            }
            catch (error) {
                return { accepted: false, runId: target.runId, error: String(error) };
            }
    ...
    ctx.tools.register(defineTool({
        ...
        async execute() {
            ...
            if (latestJob?.id) {
                const persisted = readJson(jobPathFor(latestJob.id));
                if (persisted?.request?.kind === 'user-evolution' && typeof persisted.request.planId === 'string') {
                    const plan = readJson(join(root, 'user-evolution', config.sessionId, `${persisted.request.planId}.json`));
                    const terminal = plan ? terminalJobFromPlan(plan) : null;
                    if (terminal)
                        latestJobSummary = terminal.summary;
                }
            }
            return {
                mode: config.mode,
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args) {
            ...
            if (args.action === 'rollback_recent') {
                ...
                atomicWriteJson(planPath, plan);
                if (session.recent?.jobId)
                    updateJob(session.recent.jobId, { status: 'finished', summary: `用户委托 config/${plan.target.plan.targetId}：已回滚`, error: undefined });
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args) {
            ...
            if ((resumeJobId || resumeRunId) && !resumeTarget?.runId) {
                return { accepted: false, error: resumeTarget?.error ?? 'resumeJobId or resumeRunId is invalid' };
            }
            const priorRequirements = resumeJobId
                ? readJson(jobPathFor(resumeJobId))?.request?.requirements
                : undefined;
                ...
                loopRunner = async (runnerJobId) => {
                    ...
                    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
                        ...
                        try {
                            ...
                            verdict = 'rejected';
                            reason = `adjudication exception: ${String(error)}`;
...
function parseBuilderEventCursor(value, fallbackSeq) {
    if (!value)
