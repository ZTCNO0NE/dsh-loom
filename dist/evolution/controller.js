...
import { classifyBuilderProposal } from '../deliberation/index.js';
/**
...
export class UserEvolutionController {
    ...
    plan(requirements, kind) {
        if (!requirements.trim())
            throw new Error('evolution plan requires non-empty requirements');
        const target = this.options.resolveTarget(requirements, kind);
        if (target.kind !== kind)
            throw new Error(`host resolver returned ${target.kind} for requested ${kind}`);
    ...
    read(planId) {
        const plan = readJson(this.file(planId));
        if (!plan || plan.schemaVersion !== 1)
            throw new Error(`unknown evolution plan: ${planId}`);
        return plan;
    }
    ...
    queue(planId) {
        ...
        if (plan.state !== 'planned')
            throw new Error(`evolution plan is not queueable: ${plan.state}`);
        plan.state = 'queued';
        this.write(plan);
        return plan;
    }
    ...
    cancel(planId) {
        ...
        if (plan.state !== 'queued')
            throw new Error(`evolution plan is not cancellable: ${plan.state}`);
    ...
    interrupt(planId) {
        const plan = this.read(planId);
        if (plan.state !== 'queued' && plan.state !== 'executing' && plan.state !== 'verifying')
            return plan;
    ...
    async execute(planId) {
        const plan = this.read(planId);
        if (plan.state !== 'queued' && plan.state !== 'planned')
            throw new Error(`evolution plan is not executable: ${plan.state}`);
    ...
    file(planId) { return join(this.options.root, 'user-evolution', this.options.sessionId, `${planId}.json`); }
    write(plan) {
