...
import { CandidateImporter, CandidateRegistry } from './index.js';
...
export class LoopCandidateGateway {
    ...
    async discover(requirements, context = {}) {
        if (!this.options.enabled)
            return { accepted: false, reason: 'disabled', rationale: 'allowLoopCandidates is disabled' };
        const llm = this.options.llm;
        if (!llm)
            throw new Error('loop candidate gateway: no independent builder llm available');
        ...
        for (let acquisitionAttempt = 0; acquisitionAttempt < 2; acquisitionAttempt++) {
            ...
            if (outcome.state === 'aborted' || !outcome.proposal) {
                const result = { accepted: false, reason: 'no_candidate', rationale: outcome.reason ?? 'builder declined or could not safely submit a candidate', runId: run.id };
                this.persist(result);
                return result;
            }
            ...
            if (output.candidate === null || output.candidate === undefined) {
                ...
                const result = { accepted: false, reason: 'no_candidate', rationale: rationale || 'builder submitted no candidate', runId: next.id };
                this.persist(result);
                return result;
            }
            try {
                ...
                const result = { accepted: true, rationale, manifest, runId: run.id };
                this.persist(result);
                return result;
            }
            catch (error) {
                ...
                if (acquisitionAttempt === 1) {
                    const result = { accepted: false, reason: 'acquisition_failed', rationale: String(error), runId: next.id };
                    this.persist(result);
                    return result;
                }
                run = next;
    ...
    parse(value) {
        if (!value || typeof value !== 'object' || Array.isArray(value))
            throw new Error('loop candidate gateway: builder draft must be an object');
        return value;
    }
    normalize(value) {
        ...
        if (!value.config || typeof value.config !== 'object' || Array.isArray(value.config))
            throw new Error('loop candidate gateway: config must be an object');
        if (!Array.isArray(value.capabilities))
            throw new Error('loop candidate gateway: capabilities must be an array');
        const build = value.build;
        if (!build || typeof build !== 'object' || Array.isArray(build)
            || (build.method !== 'prebuilt' && build.method !== 'sandboxed-dsh-workspace')) {
