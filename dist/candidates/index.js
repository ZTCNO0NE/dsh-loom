...
export function candidatePaths(root) {
    ...
    return {
        ...
        rollback: (id) => join(base, 'installations', `${id}.rollback.json`),
    };
}
 ...
 */
export function materializeRuntimeArtifact(sourcePackage, destination) {
    if (existsSync(destination))
        throw new Error(`candidate runtime artifact already exists: ${destination}`);
    ...
    try {
        for (const name of selected) {
            ...
            if (!existsSync(from))
                throw new Error(`candidate runtime artifact requires ${name}`);
...
function copyRegularTree(source, destination, packageRoot) {
    const stat = lstatSync(source);
    const rel = relative(packageRoot, source).split(sep).join('/') || '.';
    if (stat.isSymbolicLink())
        throw new Error(`candidate runtime artifact contains symlink: ${rel}`);
    if (stat.isFile()) {
    ...
    if (!stat.isDirectory())
        throw new Error(`candidate runtime artifact has unsupported entry: ${rel}`);
    mkdirSync(destination, { recursive: true });
    for (const name of readdirSync(source))
        copyRegularTree(join(source, name), join(destination, name), packageRoot);
}
...
export class CandidateRegistry {
     ...
     */
    recordRollback(report) {
        const registry = this.list();
        const record = registry.candidates[report.candidateId];
        if (!record)
            throw new Error(`unknown candidate: ${report.candidateId}`);
        if (record.state !== 'installed')
            throw new Error(`candidate must be installed before rollback: ${record.state}`);
