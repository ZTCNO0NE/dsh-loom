...
export class ActorEvolutionGateway {
    ...
    startConfig(requirements, plan) {
        if (!plan.targetId)
            throw new Error('config evolution requires targetId');
        ...
        if (requirements.trim())
            kernel.receiveActorMessage(run.id, { rawUserText: requirements, idempotencyKey: `initial:${run.id}` });
        return { runId: run.id };
    }
    async runConfig(runId) {
        const kernel = this.kernel();
        const context = kernel.context(runId);
        if (context.run.kind !== 'patch' || context.input.targetBefore.targetKind !== 'config')
            throw new Error('run is not a config evolution pass');
    ...
    startModule(requirements, plan) {
        if (!plan.targetId || !plan.entry)
            throw new Error('module evolution requires targetId and entry');
        const kernel = this.kernel();
        const run = kernel.create({
    ...
    async runModule(runId) {
        const kernel = this.kernel();
        const context = kernel.context(runId);
        const target = context.input.targetBefore;
        if (context.run.kind !== 'patch' || (target.targetKind !== 'tool' && target.targetKind !== 'skill'))
            throw new Error('run is not a module evolution pass');
        const requirements = typeof context.input.actor.requirements === 'string' ? context.input.actor.requirements : '';
        const execution = await runMiniSwe({
        ...
        if (!execution.submitted)
            return this.abort(kernel, runId, execution.error ?? 'mini-SWE did not submit a completed trajectory');
        try {
    ...
    startComposition(requirements, plan) {
        if (!plan.targets.length)
            throw new Error('composition evolution requires at least one host-selected target');
        const kernel = this.kernel();
        ...
        for (const target of plan.targets) {
            ...
            if (target.targetKind === 'config') {
                if (!target.before)
                    throw new Error(`composition config target ${target.id} requires before snapshot`);
    ...
    async runComposition(runId) {
        const kernel = this.kernel();
        const context = kernel.context(runId);
        const plan = context.input.targetBefore.compositionPlan;
        if (!plan || typeof plan !== 'object' || Array.isArray(plan))
            throw new Error('run is not a composition evolution pass');
        ...
        try {
            ...
            kernel.decide(runId, { kind: 'tool', action: { name: 'write_submission', proposal: proposal } });
            kernel.decide(runId, { kind: 'submit' });
            return { runId, state: 'submitted', proposal: kernel.proposal(runId) ?? undefined };
        }
        catch (caught) {
    ...
    reopen(runId, report) {
        ...
        if (prior.run.state !== 'submitted')
            throw new Error(`only submitted actor evolution runs may be reopened: ${prior.run.state}`);
        ...
        if (target.targetKind === 'config') {
            ...
            if (!priorBaseline)
                throw new Error('config reopen requires prior captured baseline');
    ...
    abort(kernel, runId, reason) {
        kernel.decide(runId, { kind: 'abort', reason });
        return { runId, state: 'aborted', reason };
    }
    acknowledgeRuntimeInbox(kernel, runId, messages, subject) {
    ...
    builderSessionId() { return scopedSessionId(this.options.sessionId, 'actor-evolution'); }
    kernel() { return new BuilderKernel(this.options.root, this.builderSessionId()); }
    paths(runId) { return builderRunPaths(this.options.root, this.builderSessionId(), runId); }
}
