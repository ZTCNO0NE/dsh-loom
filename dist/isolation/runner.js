...
export function childEnv() {
    ...
    for (const [key, value] of Object.entries(process.env)) {
        if (value === undefined)
            continue;
        if (/^(TSX|TS_NODE|NODE_OPTIONS|NODE_PATH|npm_|NPM_CONFIG_|PNPM_|INIT_CWD|npm_lifecycle_|npm_command|npm_execpath)/i.test(key)
...
export function findChangedRows(baseline, patched, targetId) {
    ...
    for (const id of ids) {
        if (id === targetId)
            continue;
        if (baselineMap.get(id) !== patchedMap.get(id))
            changed.push(id);
    }
    return changed.sort();
}
export function buildCandidateOverlay(patch, stagingRoot = '') {
 ...
 */
export function runIsolation(patch, options) {
...
function defaultDumpRunner(options) {
    return (overlays) => {
        const command = [
            ...options.dshCommand,
            '--profile',
