...
export class BuilderDriver {
        ...
        while (modelTurns < maxTurns && toolSteps <= maxTools && Date.now() - startedAt <= maxWallTimeMs) {
            ...
            try {
                ...
                if (decision.kind === 'tool' && this.options.successMarker
                    ...
                    && result && typeof result === 'object' && result.exitCode === 0
                    && `${String(result.stdout ?? '')}${String(result.stderr ?? '')}`.includes(this.options.successMarker)) {
    ...
    prompt(context) {
        ...
        if (this.options.compactPrompt)
            return this.compactPrompt(context, pendingMessages, progressBanner);
        return [
    ...
    compactPrompt(context, pendingMessages, progressBanner) {
        const diagnosisMode = context.run.mode === 'diagnosis';
        const readOnlyDiagnosis = diagnosisMode && this.options.readOnlyDiagnosis === true;
        const capabilities = new BuilderCapabilityRegistry().registerAll(this.options.capabilities ?? []);
        ...
        const evidenceSatisfied = context.run.state === 'ready_to_submit' && this.options.successMarker
            ? `Oracle evidence satisfied (marker=${this.options.successMarker}). The Kernel now accepts only write_submission, submit, or abort for this pass; do not keep exploring or edit the verified workspace.`
            : '';
        const previousAttempt = context.input.previousAttempt;
    ...
    parseDecision(text) {
        ...
        const decision = isObject(rawValue.decision)
            ? rawValue.decision
            : rawValue;
    ...
    parseTool(action) {
        if (action.name === 'read_input' && isOneOf(action.document, ['actor', 'target_before', 'previous_attempt', 'previous_run', 'world_model', 'plan', 'progress_state', 'context_index', 'provenance']))
        ...
        if (action.name === 'inspect_file' && typeof action.path === 'string')
            return { name: action.name, path: action.path };
        if (action.name === 'trace_artifact' && typeof action.artifact === 'string')
            return { name: action.name, artifact: action.artifact };
        ...
        if (action.name === 'apply_workspace_patch' && typeof action.patch === 'string')
            return { name: action.name, patch: action.patch };
        ...
        if (action.name === 'write_diagnosis_report' && isObject(action.report))
            return { name: action.name, report: action.report };
        ...
        if (action.name === 'request_input' && typeof action.question === 'string' && (action.context === undefined || typeof action.context === 'string')
            ...
            return {
                ...
                ...(action.options === undefined ? {} : { options: action.options.map((option) => ({ id: option.id, label: option.label, ...(typeof option.description === 'string' ? { description: option.description } : {}) })) }),
                ...(action.whyNow === undefined ? {} : { whyNow: action.whyNow }),
                ...(action.evidenceRefs === undefined ? {} : { evidenceRefs: action.evidenceRefs }),
        ...
        if (action.name === 'compile_loop_submission' && typeof action.rationale === 'string' && (action.expectedOutcome === undefined || typeof action.expectedOutcome === 'string'))
            return { name: action.name, rationale: action.rationale, ...(action.expectedOutcome === undefined ? {} : { expectedOutcome: action.expectedOutcome }) };
        if (action.name === 'compile_config_submission' && typeof action.rationale === 'string' && (action.expectedOutcome === undefined || typeof action.expectedOutcome === 'string'))
            return { name: action.name, rationale: action.rationale, ...(action.expectedOutcome === undefined ? {} : { expectedOutcome: action.expectedOutcome }) };
        if (action.name === 'compile_module_submission' && typeof action.rationale === 'string' && (action.expectedOutcome === undefined || typeof action.expectedOutcome === 'string'))
            return { name: action.name, rationale: action.rationale, ...(action.expectedOutcome === undefined ? {} : { expectedOutcome: action.expectedOutcome }) };
...
function builderNativeTools(readOnlyDiagnosis = false) {
...
function isCompletionDecision(decision) {
    return decision.kind === 'submit'
        || decision.kind === 'abort'
        || (decision.kind === 'tool' && decision.action.name === 'write_submission');
}
function deriveProgressBanner(journal) {
    const toolEntries = journal.filter((entry) => entry.kind === 'tool');
    const last = toolEntries.at(-1);
    if (!last || !isReadActionName(last.action) || !last.result?.observation || typeof last.result.observation !== 'object')
        return '';
    const observation = last.result.observation;
    if (observation.newInformation !== false)
        return '';
    ...
    for (let i = toolEntries.length - 2; i >= 0; i--) {
        ...
        if (entry.action !== last.action || readResultTarget(entry.result) !== target || !hasUnchangedObservation(entry.result))
            break;
        streak++;
    }
...
function hasUnchangedObservation(result) {
    if (!result || typeof result.observation !== 'object' || result.observation === null)
        return false;
    return result.observation.newInformation === false;
}
function isReadActionName(action) {
    return action === 'read_file' || action === 'read_input' || action === 'read_journal' || action === 'list_directory' || action === 'read_workspace_file';
}
function readResultTarget(result) {
    if (!result)
        return undefined;
