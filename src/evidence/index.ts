...
import { buildRuntimeDigest, type RuntimeDigest } from '../meta/digest.js'
...
function rawFile(path: string): { exists: boolean; bytes: number; lineCount: number; sha256?: string } {
  if (!existsSync(path) || !statSync(path).isFile()) return { exists: false, bytes: 0, lineCount: 0 }
...
function defaultHandoff(options: ActorEvidencePackOptions, digest: RuntimeDigest): string {
...
export function createActorEvidencePack(options: ActorEvidencePackOptions): ActorEvidencePack {
  ...
  const redactedConfig = JSON.parse(JSON.stringify(options.currentConfig)) as Record<string, unknown>
  ...
  const eventRows = readJsonl<Record<string, unknown>>(paths.events(options.root, options.sessionId))
...
export function readActorEvidencePack(path: string): ActorEvidencePack | null {
  return readJson<ActorEvidencePack>(path)
}
