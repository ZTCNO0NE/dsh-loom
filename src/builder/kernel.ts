...
export type BuilderToolAction =
  ...
  | { name: 'apply_workspace_patch'; patch: string }
  ...
  | { name: 'compile_loop_submission'; rationale: string; expectedOutcome?: string }
  /** Compile one host-materialized config update from its workspace diff. */
  | { name: 'compile_config_submission'; rationale: string; expectedOutcome?: string }
  /** Compile a host-materialized tool or skill bundle from actor-module/. */
  | { name: 'compile_module_submission'; rationale: string; expectedOutcome?: string }
...
export class BuilderKernel {
  ...
  create(input: BuilderRunInput): BuilderRunRecord {
    ...
    if (!/^builder-[0-9]+-[a-f0-9]{8}$/.test(id)) throw new Error(`invalid builder run id: ${id}`)
    if (existsSync(builderRunPaths(this.root, this.sessionId, id).record)) throw new Error(`builder run already exists: ${id}`)
  ...
  captureWorkspaceBaseline(id: string, sourceRoot = 'packages/core/agent-loop/src'): { path: string; captured: boolean } {
    const paths = builderRunPaths(this.root, this.sessionId, id)
    const source = this.workspacePath(paths.workspace, sourceRoot)
    if (!existsSync(source) || (!statSync(source).isDirectory() && !statSync(source).isFile())) throw new Error(`workspace baseline source is unavailable: ${sourceRoot}`)
    const target = resolve(paths.workspaceBaseline, sourceRoot)
    if (existsSync(target)) return { path: sourceRoot, captured: false }
  ...
  private executeTool(id: string, action: BuilderToolAction): Record<string, unknown> {
    ...
    if (action.name === 'read_file') {
      ...
      if (!existsSync(file) || !statSync(file).isFile()) throw new Error(`file is unavailable: ${file}`)
    ...
    if (action.name === 'list_directory') {
      ...
      if (!existsSync(directory) || !statSync(directory).isDirectory()) throw new Error(`directory is unavailable: ${directory}`)
    ...
    if (action.name === 'apply_workspace_patch') {
      ...
      if (check.status !== 0) throw new Error(`workspace patch rejected: ${(check.stderr || check.stdout || 'git apply check failed').trim()}`)
      const applied = spawnSync('git', ['apply', '--whitespace=nowarn', '-'], {
        cwd: paths.workspace, encoding: 'utf8', input: action.patch, maxBuffer: 256 * 1024,
      })
      if (applied.status !== 0) throw new Error(`workspace patch failed: ${(applied.stderr || applied.stdout || 'git apply failed').trim()}`)
    ...
    if (action.name === 'compile_loop_submission') {
      if (this.load(id).kind !== 'loop_candidate') throw new Error('workspace loop compilation is only available to loop_candidate runs')
      ...
      this.setPhase(id, 'ready_to_submit')
      return { written: 'compiled_loop_submission', edits: (proposal.payload as { source: { edits: unknown[] } }).source.edits.length, hash: manifest.proposalHash, manifestHash: sha256(manifest) }
    }
    if (action.name === 'compile_config_submission') {
      ...
      this.emit(id, 'proposal_drafted', { proposalHash: manifest.proposalHash, manifestHash: sha256(manifest), keys: Object.keys(proposal) })
      this.setPhase(id, 'ready_to_submit')
      return { written: 'compiled_config_submission', targetId: (proposal.payload as { targetId: string }).targetId, hash: manifest.proposalHash, manifestHash: sha256(manifest) }
    }
    if (action.name === 'compile_module_submission') {
      ...
      const payload = proposal.payload as { id: string; module: { files: Array<{ path: string; content: string }> } }
      ...
      for (const file of payload.module.files) {
        ...
        if (relative(staging, target).startsWith('..')) throw new Error(`compiled module staging path escapes root: ${file.path}`)
      ...
      this.emit(id, 'proposal_drafted', { proposalHash: manifest.proposalHash, manifestHash: sha256(manifest), keys: Object.keys(proposal) })
      this.setPhase(id, 'ready_to_submit')
      return { written: 'compiled_module_submission', targetId: (proposal.payload as { targetId: string }).targetId, hash: manifest.proposalHash, manifestHash: sha256(manifest) }
    }
  ...
  private captureWorkspaceBaselineFile(baselineRoot: string, workspace: string, file: string): void {
    if (!file.startsWith(`${resolve(workspace)}/`) || !existsSync(file) || !statSync(file).isFile()) return
   ...
   * CandidateImporter.
   */
  private compileLoopWorkspaceProposal(id: string, rationale: string, expectedOutcome?: string): Record<string, unknown> {
    if (!rationale.trim()) throw new Error('compiled loop submission requires a rationale')
    const paths = builderRunPaths(this.root, this.sessionId, id)
    const target = readJson<Record<string, unknown>>(paths.targetBefore) ?? {}
    const ref = typeof target.baselineCommit === 'string' ? target.baselineCommit : ''
    if (!/^[0-9a-f]{40}$/i.test(ref)) throw new Error('compiled loop submission requires targetBefore.baselineCommit')
    const edits: Array<{ path: string; beforeHash: string; after: string }> = []
    const visit = (directory: string) => {
      for (const entry of readdirSync(directory, { withFileTypes: true })) {
        ...
        if (entry.isDirectory()) visit(file)
          ...
          if (!relativePath.startsWith('packages/core/agent-loop/src/') || !/\.tsx?$/.test(relativePath)) continue
          const current = resolve(paths.workspace, relativePath)
          if (!existsSync(current) || !statSync(current).isFile()) throw new Error(`compiled loop edit is unavailable: ${relativePath}`)
          ...
          if (before !== after) edits.push({ path: relativePath, beforeHash: fileContentHash(before), after })
        }
      }
    }
    if (existsSync(paths.workspaceBaseline)) visit(paths.workspaceBaseline)
    if (edits.length === 0) throw new Error('compiled loop submission requires at least one captured agent-loop source edit')
    if (edits.length > 4) throw new Error('compiled loop submission exceeds the four-file edit limit')
   ...
   */
  private compileConfigWorkspaceProposal(id: string, rationale: string, expectedOutcome?: string): Record<string, unknown> {
    if (!rationale.trim()) throw new Error('compiled config submission requires a rationale')
    const paths = builderRunPaths(this.root, this.sessionId, id)
    const target = readJson<Record<string, unknown>>(paths.targetBefore) ?? {}
    const targetId = typeof target.targetId === 'string' ? target.targetId : ''
    if (!targetId) throw new Error('compiled config submission requires targetBefore.targetId')
    if (target.targetKind !== 'config') throw new Error('compiled config submission requires targetBefore.targetKind=config')
    const baseline = join(paths.workspaceBaseline, 'actor-config.json')
    const current = join(paths.workspace, 'actor-config.json')
    if (!existsSync(baseline) || !existsSync(current)) throw new Error('compiled config submission requires captured actor-config.json')
    const before = readJson<Record<string, unknown>>(baseline)
    const after = readJson<Record<string, unknown>>(current)
    if (!before || !after || Array.isArray(before) || Array.isArray(after)) throw new Error('compiled config submission requires object config snapshots')
    if (sha256(before) === sha256(after)) throw new Error('compiled config submission requires a config change')
  ...
  private compileModuleWorkspaceProposal(id: string, rationale: string, expectedOutcome?: string): Record<string, unknown> {
    if (!rationale.trim()) throw new Error('compiled module submission requires a rationale')
    const paths = builderRunPaths(this.root, this.sessionId, id)
    const target = readJson<Record<string, unknown>>(paths.targetBefore) ?? {}
    const targetId = typeof target.targetId === 'string' ? target.targetId : ''
    ...
    if (!targetId || (targetKind !== 'tool' && targetKind !== 'skill') || !entry) throw new Error('compiled module submission requires tool/skill target metadata and moduleEntry')
    const root = join(paths.workspace, 'actor-module')
    if (!existsSync(root) || !statSync(root).isDirectory()) throw new Error('compiled module submission requires actor-module directory')
    ...
    const visit = (directory: string) => {
      for (const item of readdirSync(directory, { withFileTypes: true })) {
        ...
        if (item.isDirectory()) visit(file)
          ...
          const content = readFileSync(file, 'utf8')
          if (!path || path.startsWith('../') || content.length === 0 || Buffer.byteLength(content, 'utf8') > 256 * 1024) throw new Error(`compiled module file is invalid: ${path}`)
    ...
    if (files.length === 0 || files.length > 16 || !files.some((file) => file.path === entry)) throw new Error('compiled module submission requires a bounded bundle containing its declared entry')
    if (targetKind === 'skill') {
  ...
  private readablePath(workspace: string, requestedPath: string): string {
    const normalized = requestedPath === 'workspace' || requestedPath.startsWith('workspace/')
      ? requestedPath.slice('workspace'.length).replace(/^\/+/, '')
      : requestedPath
...
function isValidSkillEntry(content: string, targetId: string): boolean {
  const frontmatter = /^---\r?\n([\s\S]*?)\r?\n---(?:\r?\n|$)/.exec(content)
...
function satisfiesProgressRequirement(requirement: BuilderProgressRequirement, action: BuilderToolAction['name']): boolean {
  ...
  if (requirement === 'write_submission') return action === 'write_submission'
  return action === 'invoke_capability'
    || action === 'run_workspace_command'
    || action === 'write_workspace_file'
    || action === 'request_input'
...
function unifiedPatchFiles(patch: string): string[] {
