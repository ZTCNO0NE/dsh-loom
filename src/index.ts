  ...
  const startNextJob = (): void => {
    ...
    void (async () => {
      if (config.notify.start) {
        ...
        injectNotice(
          job.request.kind === 'direction-diagnosis'
          ...
          job.request.kind === 'direction-diagnosis' ? 'Builder 开始方向诊断' : isLoopExploration ? 'Builder 开始探索' : evolutionPlan ? '用户演进已开始' : '开始后台优化',
        )
      }
      ...
      try {
        ...
        if (config.notify.completion && job.request.kind === 'direction-diagnosis' && persistedStatus === 'waiting_for_input') {
          ...
          injectNotice(
            card?.phase === 'waiting_for_choice'
              ? `方向诊断完成：Builder 提出了 ${card.directions.length} 个方向，等待你选择；尚未启动任何实现。`
              : '方向诊断已暂停，等待 Actor/用户补充信息；尚未启动任何实现。',
            'Builder 方向诊断待选择',
          )
        }
      } catch (error) {
        ...
        if (directionRunId) {
          try { new EvolutionTaskSessionStore(root, config.sessionId).setDiagnosisState(directionRunId, 'aborted') } catch { /* Session may already have been cancelled or replaced. */ }
    ...
    async execute(args) {
      ...
      if (session.diagnosis) {
        try {
          ...
          return cleanToolResult({ accepted: false, mode: 'direction-diagnosis', error: '方向诊断状态不可用；不会猜测或自动进入实现。' }) as unknown as JsonValue
        }
      }
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args) {
      ...
      if (args.action === 'history') {
        ...
        const plans = existsSync(historyDir)
          ? readdirSync(historyDir)
              ...
              .map((name) => readJson<UserEvolutionPlan>(join(historyDir, name)))
              .filter((plan): plan is UserEvolutionPlan => Boolean(plan?.createdAt && plan?.target))
          : []
        return cleanToolResult({
          accepted: true,
      ...
      if (session.diagnosis) {
        if (args.action === 'cancel' || args.action === 'cancel_diagnosis') {
          try {
            ...
            if (!cancelledQueuedJob) directionDiagnosisGateway.controlExploration(session.diagnosis.runId, 'cancel')
            taskSession.setDiagnosisState(session.diagnosis.runId, 'aborted')
            return cleanToolResult({ accepted: true, mode: 'direction-diagnosis', note: '已取消方向诊断；未创建 implementation plan，未修改 Actor。' }) as unknown as JsonValue
          } catch (error) {
            return cleanToolResult({ accepted: false, mode: 'direction-diagnosis', error: `取消诊断失败：${String(error)}` }) as unknown as JsonValue
          }
        }
        const status = directionDiagnosisGateway.explorationStatus(session.diagnosis.runId)
        return cleanToolResult({ accepted: true, mode: 'direction-diagnosis', task: directionDiagnosisCard({ ...status, state: session.diagnosis.state === 'aborted' ? 'aborted' : status.state }), note: '方向卡来自只读 Builder report；用户选择前不会进入实现。' }) as unknown as JsonValue
      }
      ...
      if (args.action === 'view_evidence') {
        ...
        return cleanToolResult({
          accepted: true,
          task: userEvolutionTaskCard(plan, session.active?.jobId ? readJson<PersistedJob>(jobPathFor(session.active.jobId))?.status : undefined, evolutionTaskCardExtras(session, planId)),
      ...
      if (args.action === 'cancel' || args.action === 'cancel_pending' || args.action === 'cancel_queued') {
        if (session.pending?.planId === planId && plan.state === 'planned') {
          ...
          return cleanToolResult({ accepted: true, task: userEvolutionTaskCard(plan), note: '已取消待确认任务；现在可以提出新的演进请求。' }) as unknown as JsonValue
        }
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args) {
      ...
      if (evolutionMode) {
        ...
        const currentConfig = currentConfigOf(ctx, baseline)
        let consumedDiagnosisRunId: string | undefined
        if (evolutionMode === 'plan') {
          if (!evolutionRequirements) return cleanToolResult({ accepted: false, mode: 'plan', needsClarification: true, question: '你希望 Actor 改善什么？请描述当前问题、期望行为或想新增的能力。', choices: [] }) as unknown as JsonValue
          ...
          if (selectedDirectionId) {
            ...
            if (!diagnosis || diagnosis.state !== 'waiting_for_choice') return cleanToolResult({ accepted: false, mode: 'plan', error: '当前没有等待选择的 Builder 方向诊断。' }) as unknown as JsonValue
            ...
            if (selection.kind === 'invalid') return cleanToolResult({ accepted: false, mode: 'plan', error: selection.error }) as unknown as JsonValue
            if (selection.kind === 'no_change') {
              taskSession.consumeDiagnosis(diagnosis.runId)
              return cleanToolResult({ accepted: true, mode: 'plan', noChange: true, direction: { layer: selection.direction.layer, goal: selection.direction.goal }, note: '用户选择暂不修改；没有创建 execution plan、workspace 或 proposal。' }) as unknown as JsonValue
            }
            ...
            const directionMode = args.directionMode === 'direct' || args.directionMode === 'diagnose' || args.directionMode === 'auto'
              ? args.directionMode as EvolutionDirectionMode : 'auto'
            ...
            if (route.route === 'diagnose') {
              if (!builderCredential.configured || !metaLlm) {
                return cleanToolResult({ accepted: false, mode: 'diagnose', error: `Builder credential ${builderCredential.ref} is required for direction diagnosis`, routeReason: route.reason }) as unknown as JsonValue
              }
              ...
              if (currentSession.pending || currentSession.active || (currentSession.diagnosis && currentSession.diagnosis.state !== 'aborted')) {
                return cleanToolResult({ accepted: false, mode: 'diagnose', error: '该会话已有诊断、等待确认或进行中的任务；请先查看或完成当前任务。' }) as unknown as JsonValue
              }
              const signals = observer.collect(config.thresholds)
              ...
              if (!started.accepted) return cleanToolResult({ accepted: false, mode: 'diagnose', error: started.reason }) as unknown as JsonValue
          ...
          if (clarification) {
            return cleanToolResult({ accepted: false, mode: 'plan', needsClarification: true, ...clarification, note: '尚未冻结 evidence pack，也未启动 Builder。Actor 应先向用户解释这些选择，再携带确认后的方向重新 plan。' }) as unknown as JsonValue
          }
        }
        ...
        try {
          if (evolutionMode === 'plan') {
            if (!evolutionRequirements || !targetKind) throw new Error('planning preflight did not resolve requirements and target kind')
            const currentSession = taskSession.read()
            ...
            if (existingPlanId) {
              ...
              const activeJobStatus = currentSession.active?.jobId ? readJson<PersistedJob>(jobPathFor(currentSession.active.jobId))?.status : undefined
              return cleanToolResult({
                accepted: false,
                mode: 'plan',
            ...
            if (consumedDiagnosisRunId) taskSession.replaceDiagnosisWithPending(consumedDiagnosisRunId, pendingTask)
            else taskSession.beginPending(pendingTask)
      ...
      if (args.exploreLoop === true) {
        ...
        let selectedDiagnosisRunId: string | undefined
        const selectedDirectionId = typeof args.selectedDirectionId === 'string' ? args.selectedDirectionId : undefined
        if (selectedDirectionId) {
          const taskSession = new EvolutionTaskSessionStore(root, config.sessionId)
          const diagnosis = taskSession.read().diagnosis
          if (!diagnosis || diagnosis.state !== 'waiting_for_choice') return cleanToolResult({ accepted: false, mode: 'loop-exploration', error: '当前没有等待选择的 Builder 方向诊断。' }) as unknown as JsonValue
          ...
          if (selection.kind !== 'loop_confirmation') return cleanToolResult({ accepted: false, mode: 'loop-exploration', error: '所选方向不是有效的 Loop 方向；不会跨层猜测执行。' }) as unknown as JsonValue
        ...
        if (selectedDiagnosisRunId) new EvolutionTaskSessionStore(root, config.sessionId).consumeDiagnosis(selectedDiagnosisRunId)
