...
import { runIsolation, type IsolationOptions } from '../isolation/runner.js'
...
export interface CollectFramesOptions {
  ...
  skillProbe?: (patch: MetaPatch) => { passed: boolean; name?: string } | Promise<{ passed: boolean; name?: string }>
}
 ...
 */
export async function collectFramesForPatch(
  patch: MetaPatch,
  base: VerifierInput,
  ...
  if (patch.targetKind === 'skill') {
    if (!options.skillProbe) return base
