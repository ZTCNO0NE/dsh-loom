...
export class Proposer {
  constructor(
    ...
    private options: ProposerOptions,
  ) {}
  ...
  async propose(
    ...
    const llm = this.options.llm
      ?? (this.ctx as unknown as { llm?: LlmStreamLike }).llm
    if (!llm) {
    ...
    atomicWriteJson(
      paths.selfCheck(this.options.root, this.options.sessionId),
      patch.selfCheck,
    )
    atomicWriteJson(
      paths.candidate(this.options.root, this.options.sessionId, patch.id),
      patch,
    )
    atomicWriteJson(
  ...
  private buildTaskContext(
    ...
    const profile = readJson<{ turns?: number; avgTurnMs?: number | null; maxTurnMs?: number | null; toolCalls?: number; toolErrors?: number; toolErrorRate?: number | null; perTool?: Array<{ name: string; calls: number; errors: number; avgMs: number | null }> }>(
      paths.actorProfile(this.options.root, this.options.sessionId),
    )
    const telemetryText = profile
      ? JSON.stringify(profile)
      : '(尚无观测数据)'
    const frames = readJsonl<{ ts?: string; type?: string; data?: Record<string, unknown> }>(
      paths.frames(this.options.root, this.options.sessionId),
    ).slice(-40)
  ...
  private async streamText(llm: LlmStreamLike, prompt: string): Promise<string> {
    let out = ''
    let inText = false
  ...
  private normalizePatch(parsed: Record<string, unknown>, signals: EvolutionSignal[]): MetaPatch {
    ...
    if (isLockedTarget(
      { targetKind, targetId: patch.targetId, targetName: typeof patch.targetName === 'string' ? patch.targetName : undefined },
      this.options.lockedTargets ?? DEFAULT_LOCKED_TARGETS,
    )) {
    ...
    const module = this.normalizeModule(parsed.patch as Record<string, unknown>)
    const probes = this.normalizeProbes(parsed.patch as Record<string, unknown>)
    const result: MetaPatch = {
      id,
      action,
  ...
  private normalizeProbes(patch: Record<string, unknown>): ProbeRequest[] | undefined {
  ...
  private normalizeSelfCheck(value: unknown): SelfCheck {
  ...
  private normalizePreferences(parsed: Record<string, unknown>): Preference[] {
  ...
  private normalizeTrajectory(value: unknown): ExpectedTrajectory | undefined {
    ...
    return {
      schemaVersion: PROTOCOL_VERSION,
      patchId: '',
      events: events as ExpectedTrajectory['events'],
      coverage: v.coverage as ExpectedTrajectory['coverage'] | undefined,
    }
  }

  private normalizeModule(patch: Record<string, unknown>): MetaModule | undefined {
  ...
  private writeStagingFiles(patchId: string, module: MetaModule): void {
  ...
  private writeWorldModel(value: unknown): void {
    const v = (value ?? {}) as Record<string, unknown>
    const previous = readJson<WorldModel>(paths.worldModel(this.options.root, this.options.sessionId))
    const next: WorldModel = {
      schemaVersion: PROTOCOL_VERSION,
      target: {
        id: '',
        kind: 'config',
